...
def strt(message):
	...
	try:
		...
		strtMsg = f'''
Hey [{message.from_user.first_name}](tg://user?id={message.chat.id})..!!

*📁Image to PDF Bots V²*

`I can Convert images to PDF,Convert PDF to images,Convert files to PDF`
...
IF THERE IS ANY KIND OF REPORTING, BUGS, REQUESTS, AND SUGGESTIONS PLEASE CONTACT us through :@Imagetopdfile_Supportbot
'''
			key = types.InlineKeyboardMarkup()
...
def feedback(message):
	bot.send_chat_action(message.chat.id, "typing")
	feedbackMsg = f'''
Any Suggestions..?! [Contact 👨‍💻Developers]](http://t.me/Imagetopdfile_Supportbot)
'''
...
def fls(message):
	...
	try:
		...
		suprtedPdfFile = ['.pptx', '.epub', '.xps', '.oxps', '.cbz', '.fb2']
...
`ERROR: {e}`''', message_id = picMsgId.message_id)
		...
		elif fileExt.lower() == '.pdf' and fileSize <= 100000000:
				...
				for i in range(0, noOfPages, 10):
					...
					for file in imag:
						if os.path.getsize(file) >= 10000000:
							...
							LrgFileNo += 1
							if os.path.getsize(CmpImg) >= 100000000:
								continue
...
`ERROR: {e}`''', message_id = pdfMsgId.message_id)
		...
		elif fileExt.lower() in suprtedPdfFile and fileSize <= 100000000:
